...
try:
    conn = mariadb.connect(
        user="usuario", password="password", host="localhost", database="georregias"
    )
...
# Font Awesome Icon's
external_scripts = [
    {"src": "https://kit.fontawesome.com/19f1c21c33.js", "crossorigin": "anonymous"}
]
...
def on_form_change(switches_value):
    ...
    if switches_value == [1]:
        ...
        cur.execute(
            "SELECT SUBSTRING_INDEX(location,',', 1) AS lat, SUBSTR(location, POSITION(',' IN  location)+2, LENGTH(location)) AS lon FROM record WHERE type='911';"
        )
        myresult = cur.fetchall()
        latitud = list(map(lambda x: x[0], myresult))
        longitud = list(map(lambda x: x[1], myresult))
        reportes_mapa = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 10, "opacity": 0.3, "color": "#4974a5"},
                cluster={
                    "enabled": True,
                    "size": [12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57, 60, 69, 78],
                    "step": [60, 120, 180, 240, 300, 360, 420, 480, 540, 600, 660, 720, 780, 840, 900, 960, 1020, 2100, 5200],
        ...
        cur.execute(
            "SELECT SUBSTRING_INDEX(location,',', 1) AS lat, SUBSTR(location, POSITION(',' IN  location)+2, LENGTH(location)) AS lon FROM record WHERE type='criminalidad';"
        )
        myresult = cur.fetchall()
        latitud = list(map(lambda x: x[0], myresult))
        longitud = list(map(lambda x: x[1], myresult))

        percepciones_mapa = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 14, "opacity": 0.7, "color": "#A97BB5"},
                hoverinfo="none",
            )
        )
        ...
        cur.execute(
            "SELECT SUBSTRING_INDEX(location,',', 1) AS lat, SUBSTR(location, POSITION(',' IN  location)+2, LENGTH(location)) AS lon FROM record WHERE type='seguridad';"
        )
        myresult = cur.fetchall()
        latitud = list(map(lambda x: x[0], myresult))
        longitud = list(map(lambda x: x[1], myresult))

        percepciones_seguro_mapa = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 14, "opacity": 0.7, "color": "#8bb77f"},
                hoverinfo="none",
            )
        )
        ...
        # Estaciones + Reportes
        estaciones_reportes = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 10, "opacity": 0.1, "color": "#4974a5"},
                cluster={
                    "enabled": True,
                    "size": [12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57, 60, 69, 78],
                    "step": [60, 120, 180, 240, 300, 360, 420, 480, 540, 600, 660, 720, 780, 840, 900, 960, 1020, 2100, 5200],
        ...
        estaciones_percepciones = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 14, "opacity": 0.7, "color": "#A97BB5"},
        ...
        estaciones_percepciones_seguro = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 14, "opacity": 0.7, "color": "#8bb77f"},
        ...
        reportes_percepciones = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 10, "opacity": 0.1, "color": "#4974a5"},
        ...
        reportes_percepciones_seguro = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 12, "opacity": 0.1, "color": "#4974a5"},
                cluster={
                    "enabled": True,
                    "size": [12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57, 60, 69, 78],
                    "step": [60, 120, 180, 240, 300, 360, 420, 480, 540, 600, 660, 720, 780, 840, 900, 960, 1020, 2100, 5200],
        ...
        switches_value == [1, 2, 3]
        or switches_value == [1, 3, 2]
        or switches_value == [2, 1, 3]
        or switches_value == [2, 3, 1]
        or switches_value == [3, 1, 2]
        or switches_value == [3, 2, 1]
    ):
        ...
        estaciones_reportes_percepciones = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 12, "opacity": 0.1, "color": "#4974a5"},
        ...
        switches_value == [1, 2, 4]
        or switches_value == [1, 4, 2]
        or switches_value == [2, 1, 4]
        or switches_value == [2, 4, 1]
        or switches_value == [4, 1, 2]
        or switches_value == [4, 2, 1]
    ):
        ...
        estaciones_reportes_percepciones_seguro = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 12, "opacity": 0.1, "color": "#4974a5"},
        ...
        switches_value == [1, 3, 4]
        or switches_value == [1, 4, 3]
        or switches_value == [3, 1, 4]
        or switches_value == [3, 4, 1]
        or switches_value == [4, 1, 3]
        or switches_value == [4, 3, 1]
    ):
        ...
        estaciones_percepciones_ambas = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 14, "opacity": 0.7, "color": "#8bb77f"},
        ...
        switches_value == [2, 3, 4]
        or switches_value == [2, 4, 3]
        or switches_value == [3, 2, 4]
        or switches_value == [3, 4, 2]
        or switches_value == [4, 2, 3]
        or switches_value == [4, 3, 2]
    ):
        ...
        reportes_percepciones_ambas = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 12, "opacity": 0.1, "color": "#4974a5"},
        ...
        placeholder = go.Figure(
            go.Scattermapbox(
                lon=longitud,
                lat=latitud,
                marker={"size": 12, "opacity": 0.5, "color": "#E2474B"},
                hoverinfo="none",
            )
        )
